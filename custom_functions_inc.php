<?php
	# BFE/WK:
	# Diese Datei enthält custom_function_override... Funktionen, die die entsprechenden
	# custom_function_default... Funktionen überschreiben
	# Diese Datei ist in einem Upgrade nicht enthalten, kann also auch nicht überschrieben werden.
...
/*BFE-Mod WK: 2007-01-11 ersetzt				if ( custom_field_is_linked( $t_field_id, $t_project_id ) ) {
...
// BFE-Mod WK: 2007-01-11 Ende des ersetzenden Blocks
			}
			echo $t_column_end;
		} else {
 ...
 */
# --------------------
# Modifizierte Version von print_column_summary mit Privat-Sysmbol
# Aufgerufen aus der custom_function_override_print_column_value
 ...
 * @return null
 */
function html_button_bug_clone_to_project( $p_bug_id, $t_text ) {
	html_button( 'bfe_clone_issue.php', $t_text,
				array( 'm_id' => $p_bug_id ) );
}
 ...
 * @uses config_api.php
 */
function bug_is_readonly_BFE( $p_bug_id ) {
	$t_status = bug_get_field( $p_bug_id, 'status' );

// WK/BFE: Sonderbehandlung QS-Reporter, wenn Status neu dann nicht readonly!
	if ( access_get_project_level() == 30 ) {
		if ( $t_status != 10 ) {
			return true;
		}
	}
// WK/BFE: Dies ist die Original-Funktion
...
# --------------------
			...
			if ( bug_get_field( $p_bug_id, 'status' ) < 40 ) {
				if ( access_compare_level ( access_get_project_level(), array(30, 50, 55, 70, 90) ) ) {
					...
					if ( count($t_bfe_bugs) > 1 ) {
						...
						html_button('bug_actiongroup_page.php', $t_bfe_edit_failure_class_button,
	...
	# --------------------
	# BFE/WK: eine solche Funktion gab es gar nicht....
	#
	...
	# --------------------
	# BFE/WK: und eine solche auch nicht...
	# Modifizierte Kopie von api/html_api.php:html_button()
	# Print an html button inside a form
	function bfe_fehlerklasse_button( $p_action, $p_button_text, $p_fields ) {
		$p_action		= urlencode( $p_action );
		$p_button_text	= string_attribute( $p_button_text );

		PRINT "<form method=\"post\" action=\"$p_action\">\n";

		foreach ( $p_fields as $val ) {
			$key = 'bug_arr[]';
			$val = string_attribute( $val );

			PRINT "	<input type=\"hidden\" name=\"$key\" value=\"$val\" />\n";
		}
		PRINT " <input type=\"hidden\" name=\"action\" value=\"custom_field_1\" />\n";
		PRINT "	<input type=\"submit\" class=\"button\" value=\"$p_button_text\" />\n";
		PRINT "</form>\n";
	}

	################################
	# Funktionen für custom fields #
	################################

	# -------------------
	# To use this function in a custom field type "=raum" in the possible value field.
	# WK/2007-05-09 13:48
	function custom_function_override_enum_raum() {
		...
		case 4:
		case 6:
			return "|Archiv|Assi 1|Assi 2|Assi 3|Assi 4|Atrium|Bimed Schnitt 1|Bimed Schnitt 2|Bimed Schnitt 3|CUT 1|CUT 2|CUT 3|CUT 4|CUT 5|CUT 6|CUT 7|CUT 8|ESP 1|ESP 2|ESP 3|ESP 4|ESP 5|ESP 6|ESP 7|ESP 8|ESP 9|Eventregie Bild|Eventregie Ton|Eventstudio|Foyer Weserhaus|Geräusch-Archiv|GR HSR|GR Ingest 1|GR News|GR WH 232|Grafik|HSR FS|HSR HF|Ingest 1|Ingest 2|Kamerabahnhof|Konferenz 1|Konferenz 2|Konferenz 3|Konferenz 4|Konferenz 5|Layout 1|Layout 2|Layout 4|Linear Schnitt|Multifunktionsraum|News 1|News 2|News 3|News 4|Regie 1|Regie 2|Regie 3|Regie 4|Regie 5|SAW|Sendung 1|Sendung 2|Sendung 3|Sendung 4|Sprecher 1|Sprecher 2|Sprecher 4|Studio 1|Studio 2|Studio 3|Studio 4|Studio 5|TV-Regie|TV-Studio|Z-Desk|ZGR 1|ZGR D|ZGR E|ZGR U|ZGR Z (Hörfunk)";
			break;
		case 23:
		case 24:
		case 25:
		...
		case 27:
		case 28:
			return "|Regie2|Regie 3|Regie K|FS-GRR|GRR-K|Studio 5|Studio K";
			break;
		case 35:
		case 36:
		case 37:
		case 38:
			return "|Geräteraum|BAO 1|BAO 2|Lagezentrale|Büro";
			break;
		case 43:
		case 44:
		case 45:
			return "|FSR|ZI-A|ZI-B|GR-2|GR-1";
			break;
 		default:
			return "Noch kein Raum definiert.";
			break;
		}
	}

	# -------------------
	# To use this function in a custom field type "=applikation" in the possible value field.
	# WK/2007-05-09 16:48
	function custom_function_override_enum_applikation() {
		$t_project = helper_get_current_project();
		switch($t_project) {
		case 1:
		case 3:
		case 4:
		case 6:
			return "|Adobe_After Effects|Adobe_Creative Suite|Adobe_Shake|AJA_KONA_Control Panel|Apple_Final Cut Pro|Apple_Final Cut Pro 2|AVECO_database|AVECO_Fbar|AVECO_NEWS_playout|AVECO_NEWS_rundown import|AVECO_SAW_asrun export|AVECO_SAW_device control|AVECO_SAW_playlist import|AVECO_SAW_playout|Autodesk_Maya|Chyron_iSQ|Chyron_LUCI|Chyron_Lyric Pro|daccord_broadcast event gateway|daccord_cart assist|daccord_cd assist|daccord_content browser|daccord_cut assist|daccord_daily schedule|daccord_exchange center|daccord_exchange station|daccord_filetransfer link|daccord_house keeper|daccord_library_commercial|daccord_library_graphic|daccord_library_jingle|daccord_library_music|daccord_library_music medium|daccord_library_story|daccord_library_universal medium|daccord_live link|daccord_media gateway|daccord_mediathek|daccord_news center|daccord_onair assist radio|daccord_onair spy|daccord_station id broadcaster|daccord_topic center|daccord_topic schedule|daccord_user administration|IBM_Admira|Magix_Sequoia|Merging_Pyramix|Merging_VCube|S4M_Admin - Admira|S4M_Admin - ARDFiletransfer|S4M_Admin - AutoAdmin|S4M_Admin - BalanceOmneon|S4M_Admin - BalanceSAN|S4M_Admin - CutlistFuser|S4M_Admin - DeleteJob|S4M_Admin - EDLGenerator|S4M_Admin - FESADneuExport|S4M_Admin - FlipFactoryControl|S4M_Admin - KSControl|S4M_Admin - MOSExport|S4M_Admin - MPEG-1 Index|S4M_Admin - MPEG-2 Erzeugung|S4M_Admin - P2 Balance|S4M_Admin - Pjob-Export|S4M_Admin - PlanJobExport|S4M_Admin - PlanJobImport|S4M_Admin - ShotGen|S4M_Admin - VolGen|S4M_Admin - WebExport|S4M_ClipnamenGen|S4M_Encoding-ControlMode Record/Play (HSR-Ingest)|S4M_Encoding-InteractiveMode|S4M_EssenceWebservice|S4M_HiRes-Chunk-Transfer (Omneon->SAN)|S4M_JobControl|S4M_P2 Import|S4M_PlayoutControl|S4M_PreviewClient|S4M_Searchform|S4M_VideoCutCopy";
			break;
		case 29:
		case 30:
		case 31:
