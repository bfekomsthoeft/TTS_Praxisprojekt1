...
# **********************************************************
# WK/BFE: Dies ist eine Kopie von 'bug_report_page.php'.
# Aenderungen gegenueber dem Original sind als
# '# MOD: WK/BFE' evtl. bis '# /MOD: WK/BFE' gekennzeichnet.
# **********************************************************
...
# MOD: WK/BFE: If-Abfrage ob der Master Bug existiert, geloescht, weil er immer existiert, else-Bereich geloescht
	# master bug exists...
...
# MOD: WK/BFE: Der folgende Block ist auskommentiert, damit doch in das aktuelle Projekt gekont wird.
...
# MOD: WK/BFE: Ende des auskommentierten Blocks.

# MOD: WK/BFE Den project_override auf das aktuelle Projekt setzen
# (wurde zuvor ja in login_select_proj_page.php gesetzt.)
...
# MOD: WK/BFE: In der naechtsen Zeile den Zusatztext vor der additional_info hinzugefuegt
	/*$f_additional_info		= '*** Original Project: [' . $t_bug->project_id . '] '
		. project_get_name($t_bug->project_id) . '; Original Issue: '.bug_format_id( $f_master_bug_id )." ***\n" .
						$t_bug->additional_information; */
						
	#LB/BFE 2015: project_id - Nummer vor Project-Name entfernt - https://issuetracking.bfe.tv/view.php?id=18024
	$f_additional_info		= '*** Original Project: '
		. project_get_name($t_bug->project_id) . '; Original Issue: '.bug_format_id( $f_master_bug_id )." ***\n" .
						$t_bug->additional_information; 
	...
	$t_project_id			= helper_get_current_project(); # MOD: WK/BFE war: $t_bug->project_id;

# /MOD: WK/BFE: Hier war das Ende des If-Blocks
...
<br />
<div align="center">
<form name="report_bug_form" method="post" <?php if ( $tpl_show_attachments ) { echo 'enctype="multipart/form-data"'; } ?> action="bug_report_clone.php">
<?php echo form_security_field( 'bug_report_clone' ) ?>
